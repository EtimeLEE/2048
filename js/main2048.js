...
function prepareForMobile(){
	...
	$(".grid-cell").css({
		...
		borderRadius: 0.1*cellSlideLength
	});
}
...
function updateBoardView(){
		...
		for(var j = 0; j < 4; j++){
			...
			if(board[i][j]===0){
				theNumberCell.css({
					"width" : "0px",
					"height" : "0px",
					"top" : getPosTop(i,j)+cellSlideLength/2,
					"left" : getPosLeft(i,j)+cellSlideLength/2
				});
			}else{
				theNumberCell.css({
					"width" : cellSlideLength,
	...
	$(".number-cell").each(function(index, el) {
		if($(this).text().length > 2){
			$(this).css("fontSize",0.4*cellSlideLength + "px");
		}
	});
}
